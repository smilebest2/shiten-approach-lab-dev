...
v0.2 adds `routes.json`, derived from the same source extract under the same ODbL licence. The exact offline transformation is [prepare-osaka-routes.mjs](../prepare-osaka-routes.mjs). Route candidates preserve node/way provenance and named feature positions. Runtime intersection rounding is a visual interpolation within estimated road widths. Geographic name overlays are generated guidance, not reproductions of real signs. The minimap uses standard OSM cartographic tiles online, separate from these downloadable data files; it does not redistribute downloaded OSM tiles. Leaflet has its own BSD-2-Clause licence.
